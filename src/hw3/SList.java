...
public class SList {
    ...
    public void squish() {
        ...
        SListNode cur = head;
        while (cur.next != null){
            if(cur.item.toString().equals(cur.next.item.toString())){
                cur.next = cur.next.next;
            }else
                cur = cur.next;
        }
    }
        /*SListNode temp = new SListNode(1);
        SListNode slow = temp;
        SListNode fast = head;
        while (fast != null) {
            while (fast != null && fast.next != null && fast.item.toString().equals(fast.next.item.toString())) {
                fast = fast.next;
            }
            slow.next = fast;
            slow = slow.next;
    ...
    public void twin() {
        ...
        while (cur != null) {
            ...
            SListNode twinNode = new SListNode(cur.item);
            cur.next = twinNode;
            twinNode.next = nextNode;
            cur = nextNode;
        }
    }
